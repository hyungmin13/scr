...
class Boundary_layer_energy2(Equation):
    def __init__(self, all_params):
        self.all_params = all_params
    ...
    def Loss(dynamic_params, all_params, g_batch, particles, particle_vel, boundaries, model_fns):
        ...
        b_out = model_fns(all_params, boundaries[0])
        b_out2, b_out_z2 = first_order(all_params, boundaries[1], jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[1].shape[0],1)),model_fns)
        ...
        loss_T_bz = all_params["data"]['T_ref']*b_out_z2[:,4:5]/all_params["domain"]["domain_range"]["z"][1] - boundaries[2]
        loss_T_bz = jnp.mean(loss_T_bz**2)
        ...
        total_loss = weights[0]*loss_u + weights[1]*loss_v + weights[2]*loss_w + \
                    weights[3]*loss_con + weights[4]*loss_NS1 + weights[5]*loss_NS2 + \
                    weights[6]*loss_NS3 + weights[7]*(loss_u_b + loss_v_b + loss_w_b + loss_T_b) + weights[8]*loss_ENR + weights[9]*loss_T_bz
        return total_loss
    @staticmethod
    def Loss_report(dynamic_params, all_params, g_batch, particles, particle_vel, boundaries, model_fns):
        def first_order(all_params, g_batch, cotangent, model_fns):
...
class Wall_STB_1(Equation):
    ...
    def Loss(dynamic_params, all_params, g_batch, particles, particle_vel, boundaries, model_fns):
        ...
        _, b_out_xxz = equ_func3(all_params, boundaries[0], jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(boundaries[0].shape[0],1)),
                                                        jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(boundaries[0].shape[0],1)),
                                                        jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[0].shape[0],1)), model_fns)
        _, b_out_yyz = equ_func3(all_params, boundaries[0], jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(boundaries[0].shape[0],1)),
                                                        jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(boundaries[0].shape[0],1)),
                                                        jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[0].shape[0],1)), model_fns)
        _, b_out_zzz = equ_func3(all_params, boundaries[0], jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[0].shape[0],1)),
                                                        jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[0].shape[0],1)),
                                                        jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[0].shape[0],1)), model_fns)
        b_out_z, b_out_zt = equ_func2(all_params, boundaries[0], jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[0].shape[0],1)),
                                                    jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(boundaries[0].shape[0],1)),model_fns)
        b_out, b_out_x = equ_func1(all_params, boundaries[0], jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(boundaries[0].shape[0],1)),model_fns)
        _, b_out_y = equ_func1(all_params, boundaries[0], jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(boundaries[0].shape[0],1)),model_fns)
        

        b_out2, b_out2_z = equ_func1(all_params, boundaries[1], jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[1].shape[0],1)),model_fns)
        b_out3 = model_fns(all_params, boundaries[3])
        buxs = jnp.concatenate([b_out_x[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['x'][1] for k in range(len(keys))],1)
        buys = jnp.concatenate([b_out_y[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['y'][1] for k in range(len(keys))],1)
        buzs = jnp.concatenate([b_out_z[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['z'][1] for k in range(len(keys))],1)

        buzts = jnp.concatenate([b_out_zt[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['z'][1]/all_params["domain"]["domain_range"]['t'][1] for k in range(len(keys))],1)
        buxxzs = jnp.concatenate([b_out_xxz[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['z'][1]/all_params["domain"]["domain_range"]['x'][1]/all_params["domain"]["domain_range"]['x'][1] for k in range(len(keys))],1)
        buyyzs = jnp.concatenate([b_out_yyz[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['z'][1]/all_params["domain"]["domain_range"]['y'][1]/all_params["domain"]["domain_range"]['y'][1] for k in range(len(keys))],1)
        buzzzs = jnp.concatenate([b_out_zzz[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['z'][1]/all_params["domain"]["domain_range"]['z'][1]/all_params["domain"]["domain_range"]['z'][1] for k in range(len(keys))],1)
        ...
        loss_T_b_z = all_params["data"]['T_ref']*b_out2_z[:,4:5]/all_params["domain"]["domain_range"]['z'][1] - boundaries[2]
        ...
        loss_NS3 = uts[:,2:3] + uvwp[:,0:1]*uxs[:,2:3] + uvwp[:,1:2]*uys[:,2:3] + uvwp[:,2:3]*uzs[:,2:3] + uzs[:,3:4] - all_params["data"]["viscosity"]*(uxxs[:,2:3]+uyys[:,2:3]+uzzs[:,2:3]) - uvwp[:,4:5]
        loss_NS3 = jnp.mean(loss_NS3**2)
        loss_ENR_z = buzts[:,4:5] + buzs[:,0:1]*buxs[:,4:5] + buzs[:,1:2]*buys[:,4:5] + buzs[:,2:3]*buzs[:,4:5] - all_params["data"]["viscosity"]*(buxxzs[:,4:5]+buyyzs[:,4:5]+buzzzs[:,4:5])/0.25
        loss_ENR_z = jnp.mean(loss_ENR_z**2)

        total_loss = weights[0]*loss_u + weights[1]*loss_v + weights[2]*loss_w + \
                    weights[3]*loss_con + weights[4]*loss_NS1 + weights[5]*loss_NS2 + \
                    weights[6]*loss_NS3 + weights[7]*loss_ENR_z + weights[8]*(loss_u_b + loss_v_b + loss_w_b) + \
...
class Wall_STB_2(Equation):
    ...
    def Loss(dynamic_params, all_params, g_batch, particles, particle_vel, boundaries, model_fns):
        ...
        loss_T_b_1 = all_params["data"]['T_ref']*b_out2[:,4:5] - boundaries[4]
        loss_T_b_1 = jnp.mean(loss_T_b_1**2)
...
class Wall_STB_3(Equation):
    ...
    def Loss_report(dynamic_params, all_params, g_batch, particles, particle_vel, boundaries, model_fns):
        ...
        b_out2, b_out2_z = equ_func1(all_params, boundaries[1], jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[2].shape[0],1)),model_fns)
        b_out3, b_out3_z = equ_func1(all_params, boundaries[3], jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(boundaries[3].shape[0],1)),model_fns)
        buxs = jnp.concatenate([b_out_x[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['x'][1] for k in range(len(keys))],1)
        buys = jnp.concatenate([b_out_y[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['y'][1] for k in range(len(keys))],1)
        buzs = jnp.concatenate([b_out_z[:,k:(k+1)]*all_params["data"][keys[k]]/all_params["domain"]["domain_range"]['z'][1] for k in range(len(keys))],1)
