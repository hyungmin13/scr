...
def Derivatives(dynamic_params, all_params, g_batch, model_fns):
    ...
    Tx = uyts[:,2]-uzts[:,1] + uvwp[:,0]*(uxys[:,2] - uxzs[:,1]) + uvwp[:,1]*(uyys[:,2] - uyzs[:,1]) + uvwp[:,2]*(uyzs[:,2] - uzzs[:,1]) \
         - (uys[:,2] - uzs[:,1])*uxs[:,0] - (uzs[:,0] - uxs[:,2])*uys[:,0] - (uxs[:,1] - uys[:,0])*uzs[:,0] \
         - all_params["data"]["viscosity"]*(uxxys[:,2] + uyyys[:,2] + uyzzs[:,2] - uxxzs[:,1] - uyyzs[:,1] - uzzzs[:,1])
    
    Ty = (uys[:,2] - uzs[:,1])*uxs[:,1] + (uzs[:,0] - uxs[:,2])*uys[:,1] + (uxs[:,1] - uys[:,0])*uzs[:,1] \
         + all_params["data"]["viscosity"]*(uxxzs[:,0] + uyyzs[:,0] + uzzzs[:,0] - uxxxs[:,2] - uxxys[:,2] - uxzzs[:,2]) \
         - uzts[:,0] + uxts[:,2] - uvwp[:,0]*(uxzs[:,0] - uxxs[:,2]) - uvwp[:,1]*(uyzs[:,0] - uxys[:,2]) - uvwp[:,2]*(uzzs[:,0] - uxzs[:,2])
    return uvwp, Tx, Ty
